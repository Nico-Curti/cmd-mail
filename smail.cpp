// g++ smail.cpp -o smail -std=c++11 -O3 -march=native
...
std :: string joinString (const char & sep, const std :: string & tk1, const std :: string & tk2)
{
  return tk1 + sep + tk2;
}
...
std :: string joinString (const char & sep, const std :: string & tk1, const std :: string & tk2, Args... args)
{
...
int send_mail(const std::string &smtp, const std::string &port, const std::string &from, const std::string &to, const std::string &psswrd, const std::string &msg_file)
{
...
void parse_yml (const std :: string & filename, config & yml)
{
	...
	if(!is)
  {
    std :: cerr << "Parse Yml error! File not found. Given: " << filename << std :: endl;
    std :: exit(1);
  }
...
void sender( config & conf, std :: string & FROM, std :: string & TO, const std :: string & cmd )
{
	std :: string msg_file = "mail.tmp";
	std :: string script = cmd.substr(0, cmd.find_first_of(" "));
	std :: string password;
...
static void show_usage (const std :: string & exe)
{
...
int main (int argc, char ** argv)
{
	...
	std :: string to = std :: string(argv[2]);
